...
const createStaticPathArrayForTagsAndPage = ({
	...
	uniqueTags.forEach((unique_tag: string) => {
		...
		do {
			var paginatedArray: Array<PostType> = getPaginatedArray(sortedPosts, i, PAGE_SIZE)
			paginatedArray = hydratePaginatedPostArray(paginatedArray, assetMap, assets)
			var path = {
				params: {
...
const SortPostsByDate = (AllPosts: Array<PostType>): Array<PostType> => {
	var sortedPosts = [...AllPosts]
	sortedPosts.sort((a: PostType, b: PostType) => {
		return (new Date(b.frontmatter.pubDate) as any) - (new Date(a.frontmatter.pubDate) as any)
	})
	return sortedPosts
}
