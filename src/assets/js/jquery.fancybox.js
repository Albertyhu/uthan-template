	...
	$.extend(FancyBox.prototype, {
			...
			if (!$.fancybox.getInstance()) {
				...
				if (/iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream) {
					...
					if (scrollbarWidth === undefined) {
						$scrollDiv = $('<div style="width:50px;height:50px;overflow:scroll;" />').appendTo(
							'body'
						)
		...
		jumpTo: function (pos, duration, slide) {
			...
			if (current.isMoved) {
				...
				transitionProps =
					'fancybox-animated ' + transitionProps + ' fancybox-fx-' + current.opts.transitionEffect
	...
	Guestures.prototype.limitMovement = function () {
		...
		if (currentWidth > canvasWidth) {
			...
			if (distanceX > 0 && newOffsetX > minTranslateX) {
				newOffsetX =
					minTranslateX - 1 + Math.pow(-minTranslateX + currentOffsetX + distanceX, 0.8) || 0
			}

			//    <-
			if (distanceX < 0 && newOffsetX < maxTranslateX) {
				newOffsetX =
					maxTranslateX + 1 - Math.pow(maxTranslateX - currentOffsetX - distanceX, 0.8) || 0
			}
		}
		...
		if (currentHeight > canvasHeight) {
			...
			if (distanceY > 0 && newOffsetY > minTranslateY) {
				newOffsetY =
					minTranslateY - 1 + Math.pow(-minTranslateY + currentOffsetY + distanceY, 0.8) || 0
			}

			//   /\
			if (distanceY < 0 && newOffsetY < maxTranslateY) {
				newOffsetY =
					maxTranslateY + 1 - Math.pow(maxTranslateY - currentOffsetY - distanceY, 0.8) || 0
			}
		}
			...
			if (self.instance.current) {
				...
				if (
					...
					(thumbPos.left < $list.parent().scrollLeft() ||
						thumbPos.left >
							$list.parent().scrollLeft() + ($list.parent().width() - thumb.outerWidth()))
				) {
...
;(function (document, window, $) {
	...
	function parseUrl() {
		...
		var rez = hash.split('-')
		var index =
			rez.length > 1 && /^\+?\d+$/.test(rez[rez.length - 1]) ? parseInt(rez.pop(-1), 10) || 1 : 1
